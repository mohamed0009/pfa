...
export class AiChatComponent implements OnInit, AfterViewChecked, OnDestroy {
  ...
  loadConversations(): void {
    this.chatService.getConversations().subscribe({
      ...
      error: (err) => {
        ...
        // Don't alert here to avoid annoying popups on load, but log it
      }
    });
  }
  ...
  createNewConversation(): void {
    ...
    console.log('Creating conversation with title:', this.newConversationTitle);
    this.chatService.createConversation(this.newConversationTitle).subscribe({
      next: (conversation) => {
      ...
      error: (err) => {
        ...
        alert('Erreur lors de la création de la conversation. Vérifiez que le backend est démarré.');
      }
    });
  }
